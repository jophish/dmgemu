...
int op_pop_qq(emu *gb_emu_p, int qq_code) {
  ...
  *reg_pair_p = (qq_code == QQ_PAIR_CODE_AF) ? read_16(gb_emu_p, sp) & 0xfff0 : read_16(gb_emu_p, sp);
  set_SP(z80_p, sp + 2);
  return 0;
}
...
int op_rr_ind_hl(emu *gb_emu_p) {
  ...
  if ((val = read_8(gb_emu_p, get_HL(z80_p))) < 0)
    return val;
  uint8_t result = (val >> 0x01) | (get_flag_C(z80_p) << 7);
